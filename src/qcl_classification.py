...
class QclClassification:
    ...
    def pred(self, theta):
        ...
        if self.n_jobs == 1:
            ...
            with tqdm_joblib(tqdm(total=len(st_list), desc="pred instance")):
    ...
    def B_grad(self, theta):
        ...
        if self.n_jobs == 1:
            grad = [single(i) for i in tqdm(indices, desc="B_grad")]
        else:
            with tqdm_joblib(tqdm(total=len(theta), desc="B_grad")):
